...
class FormatHelper {
    ...
    public static function unformatHora($hora) {
     ...
     /**
    * @tutorial: Formataçao para datatime
    * FormatHelper::dataTimeInversaToNormal
    * @package system
    * @subpackage helpers
    * @access public
    * @param type $dataTime
    ...
    */
   public static function dataTimeInversaToNormal($dataTime) {
    ...
    public static function formatTelefone($tel) {
        ...
        } elseif(strlen($telefone_unformat) == 11) {
            $telefone_prefixo = substr($telefone_unformat, 0, 2);
            $telefone_pri = substr($telefone_unformat, 2, 5);
            $telefone_seg = substr($telefone_unformat, 7, 4);
    ...
    /**
     * @tutorial: Formatação de numero monetário para numero decimal
     * FormatHelper::unFormatNumeroMonetario
     * @package system
     * @subpackage helpers
     * @access public
     * @param type $valor
     ...
     */
    public static function unFormatNumeroMonetario($valor) {
        if ($valor === null || $valor === '') {
            return 0.00;
        } else {
            $arrayValor = explode(',', $valor);
            $arrayValor[0] = str_replace('.', '', $arrayValor[0]);
     ...
     * @return Array $array
     */
    public static function gerarStringIN(array $array, $param = null) {
    ...
    public static function alias($str) {
        $str = preg_replace("[^a-zA-Z0-9_]", "", strtr($str, "áàãâéêíóôõúüçÁÀÃÂÉÊÍÓÔÕÚÜÇ ", "aaaaeeiooouucAAAAEEIOOOUUC_"));
        return strtolower($str);
    }

    public static function convertTextToURL($str) {

        $str = FormatHelper::alias($str);
        $array = explode("_", $str);
        $url = "";
    ...
    /**
     * @tutorial: Formataçao para numero do ajuste
     * FormatHelper::formatNumeroAjuste
     * @package system
     * @subpackage helpers
     * @access public
     * @param type $data
     * @return string $data_format
     */
    public static function formatNumeroAjuste($numero) {

        $numero_unformat = $numero;
        $ajuste = substr($numero_unformat, 0, 4);
        $ano = substr($numero_unformat, 4, 4);
        $ajuste_format = $ajuste . "/" . $ano;

        return $ajuste_format;
    }
