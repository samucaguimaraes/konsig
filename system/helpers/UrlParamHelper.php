...
class UrlParamHelper {
    ...
    private function setUrl() {
        if (isset($_GET['url'])) {
            ...
            $_SERVER['QUERY_STRING'] = (!empty($_SERVER['QUERY_STRING']) ) ? $_SERVER['QUERY_STRING'] : PAGE_INDEX;
    ...
    public function getParam($name = null, $dencrypt = true) {
        if ($name !== null && !isset($this->params[$name])) {
            return false;
        } elseif ($name !== null && isset($this->params[$name])) {
            return ($dencrypt === false) ? $this->params[$name] : SecurityEncryptionHelper::getDecrypt($this->params[$name]);
        } else {
            return $this->getParams($dencrypt);
        }
    }

    private function getParams($dencrypt = true) {
        if (is_array($this->params) && $dencrypt === true) {
            $params = array();
            foreach ($this->params as $key => $value) {
                $params[$key] = SecurityEncryptionHelper::getDecrypt($value);
            }
            return $params;
        } else {
            return $this->params;
