...
class RegistryORM {
    ...
    private function isNotice($nameDal, &$conn) {
        if (!$conn) {
            ...
            RedirectorHelper::goToControllerAction("Warning", "database");
        }
    }
    ...
    public function getClassConn(&$nameDal) {
        ...
        if (!$dados) {
            ...
            $classConn->user = (isset($dados['user'])) ? $dados['user'] : null;
            $classConn->password = (isset($dados['password'])) ? $dados['password'] : null;
            return $classConn;
        }
    }

    private function getDadosConexao($nameDal) {
        $ini = parse_ini_file(PATH_CONFIG.'dal.ini', true);
