...
class MySqlPdoStrategy {
    ...
    /**
     * Converte um array de objeto em colmap
     * @param type $arrayObject
     * @return type 
     */
    public function objectAllToArrayColmap($arrayObject) {
        $dados = array();
        foreach ($arrayObject as $object) {
            $dados[] = $this->objectToArrayColmap($object);
        }
        return $dados;
    }
                ...
                if (isset($array[$atributo]) && is_array($array[$atributo])) {
                    ...
                    if (isset($propriedades['OneToMany'])) {
                        ...
                        foreach ($array[$atributo] as $key => $item) {
                            ...
                            $arrayObjOneToMany[] = $valideOneToMany[1];
                        }

                        $objeto->$setValue($arrayObjOneToMany);
                    } elseif (isset($propriedades['ManyToMany'])) {
            ...
            if (!$colmap) {
                if (isset($propriedades['OneToMany'])) {
                    if (isset($dados[$atributo]) && $dados[$atributo] !== "" && $dados[$atributo] !== null) {
                        $strategy = StrategyORM::getStrategy($propriedades['OneToMany']['objeto']);
                        $OneToMany[$atributo] = $strategy->objectAllToArrayColmap($get);
                        unset($strategy);
                    }
                } elseif (isset($propriedades['ManyToMany'])) {
                    if (isset($dados[$atributo]) && $dados[$atributo] !== "" && $dados[$atributo] !== null) {
